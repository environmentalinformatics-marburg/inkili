...
ldr_query <- function(plotID, crdnt_x, crdnt_y, radius){
  ...
  ldr_sapply <- sapply(seq(length(crdnt_x)), function(i) {
    ...
    #calculate standard deviation of all first returns in one plot
    ldr_sd_firsts <- sd(ldr_pnts_all$z[which(ldr_pnts_all$returnNumber == 1)])
    ...
    return(list(max_hght = ldr_max_hght, sd = ldr_sd_hght, mdn = ldr_mdn_rtrn,
                max_angl = ldr_max_angl, sd_firsts = ldr_sd_firsts, 
